...
export default class SelectPorts {
    ...
    _injectPortSelect() {
        ...
        const options = `${selectPorts
            .map(port => `<option data-subtext="${port.nation}" value="${port.coord}" data-id="${port.id}">${port.name}</option>`)
            .join("")}`;
    ...
    _setFrontlinePorts(type, nation) {
        ...
        const ownPorts = new Set(this._frontlinesData[type][nation].flatMap(frontlinePort => frontlinePort.value.map(d => d)));
    ...
    _filterCaptured(begin, end) {
        const portData = this._ports.portDataDefault.filter(port => moment(port.lastPortBattle, "YYYY-MM-DD HH:mm").isBetween(begin, end, null, "(]"));
        this._ports.portData = portData;
        this._ports.showRadius = "";
        this._ports.update();
    }
