...
export default class ListRecipes {
    ...
    async _loadAndSetupData() {
        try {
            this._moduleData = (await import("~Lib/gen-generic/modules.json")).default;
            this._recipeData = (await import("~Lib/gen-generic/recipes.json")).default.recipe;
        }
        catch (error) {
    ...
    _recipeSelected(event) {
        const recipeId = Number($(event.currentTarget)
            .find(":selected")
            .val());
